...
@test "deploy csi-driver-lvm-controller" {
    ...
    run helm install ${DOCKER_TAG} --wait /files/charts/csi-driver-lvm --set pluginImage.tag=${DOCKER_TAG} --set provisionerImage.tag=${DOCKER_TAG} --set lvm.devicePattern="${DEVICEPATTERN}" --set pluginImage.pullPolicy=${PULL_POLICY} --set provisionerImage.pullPolicy=${PULL_POLICY} --set lvm.driverName="${DOCKER_TAG}.lvm.csi.metal-stack.io" --set lvm.storageClassStub="${DOCKER_TAG}-csi-driver-lvm" -n ${DOCKER_TAG}
    [ "$status" -eq 0 ]
}
